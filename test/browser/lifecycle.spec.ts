...
const snapshot = (session: ExampleSession): Promise<Snapshot> =>
  session.page.evaluate(() => {
    const api = (window as never as ExampleWindow).pointCloudExample;
...
const untilRendererGains = async (
  ...
  throw new Error(`no tile reached the renderer\nlast keys: ${shown(last)}`);
};
    ...
    it(`keeps every tile it was handed and restores them on showing: ${cloud.name}`, async () => {
      await withSession(
        ...
        async (session) => {
          ...
          expect(
            atHide.stats.controller!.physicalTileOperations,
          ).toBeGreaterThan(0);
    ...
    it(`sheds residency into the byte-bounded cache: ${cloud.name}`, async () => {
      await withSession(
        ...
        async (session) => {
          ...
          expect(atDeactivate.stats.controller!.residentTiles).toBeGreaterThan(
            0,
          );
