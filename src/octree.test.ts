...
describe("key string codec", () => {
  ...
  it("reads a level off an encoded key without decoding the rest", () => {
    for (const key of [
      ROOT_KEY,
      { level: 1, x: 1, y: 0, z: 1 },
      { level: 10, x: 1023, y: 512, z: 0 },
    ]) {
      expect(levelFromString(keyToString(key))).toBe(key.level);
    }
    // Sorting is not the place to reject a key: unreadable input compares
    // equal rather than throwing out of a comparator.
    expect(levelFromString("nonsense")).toBeNaN();
    expect(levelFromString("")).toBeNaN();
  });
...
describe("children derivation", () => {
  it("produces 8 distinct children, all mapping back to the parent", () => {
    ...
    for (const child of children) {
      ...
      expect([child.x >> 1, child.y >> 1, child.z >> 1]).toEqual([
        key.x,
        key.y,
        key.z,
      ]);
